...
if (isset($_GET['delete'])) {
    ...
    $deleteSql = "DELETE FROM bon WHERE numbon = ?";
    ...
    if ($deleteResult) {
        header("Refresh:0; url=./deletebon.php"); // Redirect to the same page after successful deletion
    }
}
?>
            ...
            if ($result && mysqli_num_rows($result) > 0) { // Check if query result is valid
                while ($row = mysqli_fetch_assoc($result)) {
                    ...
                    <tr>
                        ...
                        <td>
                            <a href="?delete=<?php echo $row['numbon']; ?>" onclick="return confirm('Vous êtes sûr de supprimer ce document?')">
